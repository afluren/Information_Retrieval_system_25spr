    ...
    for i in range(1):
        ...
        with open(f"deduped_html/nankai{file_id}.jsonl", "r", encoding="utf-8") as f:
            ...
            while True:
                ...
                web_anchor_text = " ".join(
                    anchor["text"] for anchor in web_data["anchors"] if "text" in anchor)
                # 文本拼接
                try:
                    full_text = " ".join(
                        [web_text, web_title, web_anchor_text])
                except Exception as e:
                    print(e)
                    continue
                # 分词
                tokens = tokenize(full_text)
                # 保存文档元信息，方便未来查找。区分文件和网页
                if "filetype" in web_data:
                    doc_info[doc_id] = {
                        "title": web_data["title"],
                        "url": web_data["url"],
                        "filetype": web_data["filetype"],
                        "length": len(tokens),
                        "offset": offset
                    }
                else:
            ...
            pbar.close()
            # 保存倒排索引到磁盘
            with open(f"inverted_index/nankai{file_id}.pkl", "wb") as f:
...
def build_inverted_index():
    ...
    with Manager() as manager:
        ...
        # 词典，token -> token_id
        token_to_id = manager.dict()
        # 列表，token_id -> token
        id_to_token = manager.list()
        # 文档信息，doc_id -> doc_info
        doc_info = manager.dict()
        # 锁
        ...
        for p in process:
            p.join()
        # 保存词典、列表、文档信息
        with open("inverted_index/token_to_id.pkl", "wb") as f:
            pickle.dump(dict(token_to_id), f)
        with open("inverted_index/id_to_token.pkl", "wb") as f:
            pickle.dump(list(id_to_token), f)
        with open("inverted_index/doc_info.pkl", "wb") as f:
            pickle.dump(dict(doc_info), f)
        ...
        for i in range(num_workers):
            with open(f"inverted_index/nankai{i}.pkl", "rb") as f:
                ...
                inverted_indexs.append(inverted_index)
    ...
    for i in range(32):
        ...
        for token, doc_dict in inv.items():
            for doc_id, doc_info in doc_dict.items():
                ...
                cur.execute(
                    "SELECT freq, positions FROM inverted_index WHERE token=? AND doc_id=?", (token, doc_id))
                row = cur.fetchone()
                ...
                cur.execute("REPLACE INTO inverted_index VALUES (?, ?, ?, ?)",
...
if __name__ == '__main__':
    ...
    merge_indexes_to_sqlite("inverted_index", "inverted_index.db")
    print("Done.")
